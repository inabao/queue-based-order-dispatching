...
public class IdleTimeGreedy extends BaseAlgorithm{
    private String info;

    private boolean isRealDemand;

    public IdleTimeGreedy(boolean isRealDemand, String info){
        ...
        for (int i = 0; i < ZoneDemandTable.TOTAL_ZONE_COUNT; i++) {
            ...
            while (!zoneDrivers.isEmpty() && !zoneOrders.isEmpty()){
                ...
                for (Driver driver: zoneDrivers){
                    selectedOrder = findShortestOrder(zoneOrders,  zoneIdleTimeMap, oracle, instance.taxiWatchdog, currentTimeOffset, zoneDriverCount );
